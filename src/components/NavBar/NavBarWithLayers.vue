  ...
  <div class="inroom-navbar">
    <Menubar :model="menuItems" class="h-full w-full flex justify-between items-center">
      ...
      <template #start>
        ...
        >
          <span class="text-white text-2xl font-bold tracking-wide lexend-exa group-hover:text-pink-100 transition-colors duration-200">
            inRoom
          </span>
          <Avatar :image="pinImage" class="ml-2" size="large" shape="circle" />
        </router-link>
      </template>
      ...
      <template #end>
        <div class="flex items-center gap-4 z-10">
          ...
          <!-- Error state -->
          <div v-else-if="menuStore.getMenuError" class="glass-effect text-red-200 text-sm px-3 py-2 rounded-lg">
            <i class="pi pi-exclamation-triangle mr-2"></i>
            Error: {{ menuStore.getMenuError }}
          </div>
          
          <!-- Perfil del usuario -->
          <div v-if="USE_MOCK_DATA.value ? isAuthenticated : authStore.isAuthenticated" class="relative">
            <button 
            ...
            >
              ...
              <div class="px-4 py-3 border-b border-gray-200/20">
                <div class="flex items-center space-x-3">
                  <div class="w-12 h-12 bg-inroom-gradient rounded-full flex items-center justify-center">
