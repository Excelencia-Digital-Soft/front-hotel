...
interface UserData {
  ...
  institucionIDActiva: number | null;
}

interface AuthState {
  token: string | null;
  userData: UserData | null;
  uiSelectedInstitucionId: number | null;
}
...
export const useAuthStore = defineStore('auth', {
  state: (): AuthState => ({
    ...
    uiSelectedInstitucionId: (() => {
      const id = localStorage.getItem('uiSelectedInstitucionId');
      return id ? parseInt(id) : null;
    })(),
  }),

  persist: false, // Disable persist plugin for now due to TypeScript compatibility issues
  ...
  actions: {
    ...
    selectInstitucionForUI(institucionId: number) {
